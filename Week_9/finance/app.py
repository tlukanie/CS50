...
def buy():
    ...
    if request.method == "POST":
        ...
        #access the integer cash
        current_cash = current_balance[0]['cash'];
        return render_template("index.html", stock=quoted['name'], price=quoted['price'], cash=current_cash)
    else:
...
def quote():
    ...
    if request.method == "GET":
        ...
        if quoted == None:
            ...
            return render_template("quoted.html", stock=quoted['name'], price=quoted['price'])
...
def register():
    ...
    if request.method == "POST":
        """render an appology if the username is blank"""
        if not request.form.get("username"):
        ...
        rows = db.execute("SELECT * FROM users WHERE username = ?", username)
        #remember which user has registered and automatically logged in
        session["user_id"] = rows[0]["id"]
